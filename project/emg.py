...
def compute_emg_zscore(epochs: mne.Epochs) -> pd.DataFrame:
    ...
    # Rectify 
    emg_data = np.abs(epochs_emg.get_data()) 
    ...
    # Calculate baseline period for each epoch
...
def compute_and_save_emg_summary(df: pd.DataFrame, subject: str) -> None:
    ...
    # Calculate the Difference (Symmetry - Random)
    try:
